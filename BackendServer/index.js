...
app.post("/createUser", (req, res) => {
  console.log(req.body)
    userModel.create(req.body)
        .then(user =>{ console.log(`User ${user.vendor_name}email : ${user. vendor_email}inserted successfully`); res.json(user)})
...
app.put("/updateBank/:bankID", async (req, res) => {
  const { bankID } = req.params;
  try {
    const updatedBank = await BankModel.findOneAndUpdate({ bankID: bankID }, req.body, { new: true });
    if (!updatedBank) {
      return res.status(404).json({ error: "Bank entry not found" });
    }
    console.log(`Bank entry with ID ${updatedBank.bankID} updated successfully`);
...
app.delete("/deleteBank/:bankID", async (req, res) => {
    ...
    try {
      ...
      console.log(`Bank entry with ID ${deletedBank.bankID} deleted successfully`);
